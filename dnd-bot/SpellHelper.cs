...
namespace dnd_bot
{
    ...
    {
        public async void printSpell(SocketCommandContext Context, string SpellName)
        {
            var temp = formatSpellName(SpellName);
            var deserializedData = tryGetSpell(temp);
        ...
        public static Root tryGetSpell(string formatted)
        {
            System.Net.WebClient wc = new System.Net.WebClient();
            Root deserializedData;
            string webData;
            try
            {
                webData = wc.DownloadString("https://www.dnd5eapi.co/api/spells/" + formatted + '/');
                deserializedData = JsonConvert.DeserializeObject<Root>(webData);
            }
            catch
            {
                return null;
        ...
        public static string formatSpellName(string spellName)
        {
            return spellName.ToLower().Trim().Replace(' ', '-').Replace('/', '-');
        }
